...
console.info("- - - - - - - - - -")
var catalogoCompleto = filtrar("carousel-portrait");
console.log('catalogoCompleto', catalogoCompleto)
...
const organizar = lista =>
	lista.reduce((prev, item) => {
    	...
    	//const letter = cleanString(item.toLowerCase());
        const letter = cleanString(item.categories.toLowerCase().split(',')[0]);
        if (prev[letter]) {
    ...
    console.log(organizar( catalogoCompleto ))
    //console.log(organizar( cc ))
